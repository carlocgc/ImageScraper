...
namespace ImageScraperTests
{
    ...
    TEST_CLASS(RateLimitedHttpClientTests)
    {
    public:
    TEST_METHOD(RateLimitedHttpClient_Admits_Up_To_N_Requests_And_Waits_For_The_Next_Slot)
    {
        ...
        Assert::IsTrue(  client.Get( req, "fast" ).m_Success );
        Assert::IsTrue(  client.Get( req, "fast" ).m_Success );
        Assert::IsTrue(  client.Get( req, "fast" ).m_Success );
        ...
        Assert::IsTrue(  elapsed >= std::chrono::milliseconds( 900 ) );
    
        Assert::IsTrue(  inner->m_CallCount == 4 );
        Assert::IsTrue(  inner->m_LastRateLimitKey == "fast" );
    }
    
    TEST_METHOD(RateLimitedHttpClient_With_No_Spec_Under_Any_Key_Passes_Through_Unrestricted)
    {
        auto inner = std::make_shared<MockHttpClient>( );
        inner->m_Response = MakeSuccess( );
        ...
        for( int i = 0; i < 20; ++i )
        {
            Assert::IsTrue(  client.Get( req, "anything" ).m_Success );
        }
        const auto elapsed = std::chrono::steady_clock::now( ) - start;
        Assert::IsTrue(  elapsed < std::chrono::milliseconds( 500 ) );
        Assert::IsTrue(  inner->m_CallCount == 20 );
    }
    
    TEST_METHOD(RateLimitedHttpClient_Empty_Key_Falls_Back_To_Default_Bucket)
    {
        auto inner = std::make_shared<MockHttpClient>( );
        inner->m_Response = MakeSuccess( );
        ...
        const auto start = std::chrono::steady_clock::now( );
        Assert::IsTrue(  client.Get( req, "" ).m_Success );
        Assert::IsTrue(  client.Get( req, "" ).m_Success );
    ...
    TEST_METHOD(RateLimitedHttpClient_Unknown_Key_Falls_Back_To_Default_When_Present)
    {
        auto inner = std::make_shared<MockHttpClient>( );
        inner->m_Response = MakeSuccess( );
        ...
        HttpRequest req = MakeRequest( );
        Assert::IsTrue(  client.Get( req, "unregistered_key" ).m_Success );
        const auto start = std::chrono::steady_clock::now( );
        Assert::IsTrue(  client.Get( req, "unregistered_key" ).m_Success );
        const auto elapsed = std::chrono::steady_clock::now( ) - start;
        Assert::IsTrue(  elapsed >= std::chrono::milliseconds( 900 ) );
    ...
    TEST_METHOD(RateLimitedHttpClient_Returns_Synthetic_Failure_On_Cancellation)
    {
        auto inner = std::make_shared<MockHttpClient>( );
        inner->m_Response = MakeSuccess( );
        ...
        Assert::IsTrue(  client.Get( req, "tight" ).m_Success );
        ...
        Assert::IsFalse(  response.m_Success );
        Assert::IsTrue(  response.m_StatusCode == -1 );
        Assert::IsTrue(  response.m_Error == "cancelled" );
        Assert::IsTrue(  elapsed < std::chrono::milliseconds( 1500 ) );
        // Inner client should NOT have been called for the cancelled request.
        Assert::IsTrue(  inner->m_CallCount == 1 );
    }
    
    TEST_METHOD(RateLimitedHttpClient_Enforces_Stacked_Windows_Independently)
    {
        auto inner = std::make_shared<MockHttpClient>( );
        inner->m_Response = MakeSuccess( );
        ...
        for( int i = 0; i < 5; ++i )
        {
            Assert::IsTrue(  client.Get( req, "stacked" ).m_Success );
        }
        const auto afterFirst5 = std::chrono::steady_clock::now( ) - start;
        Assert::IsTrue(  afterFirst5 < std::chrono::milliseconds( 500 ) );
        ...
        Assert::IsTrue(  afterFirst8 >= std::chrono::milliseconds( 900 ) );
        Assert::IsTrue(  afterFirst8 < std::chrono::milliseconds( 2500 ) );
        ...
        Assert::IsTrue(  afterFirst9 >= std::chrono::milliseconds( 2800 ) );
    
        Assert::IsTrue(  inner->m_CallCount == 9 );
    }
    
    TEST_METHOD(RateLimitedHttpClient_Fires_Sink_Callback_When_Wait_Exceeds_Threshold)
    {
        auto inner = std::make_shared<MockHttpClient>( );
        inner->m_Response = MakeSuccess( );
        ...
        HttpRequest req = MakeRequest( );
        Assert::IsTrue(  client.Get( req, "slow" ).m_Success );
        Assert::IsTrue(  sink->m_WaitCalls.load( ) == 0 );
    
        Assert::IsTrue(  client.Get( req, "slow" ).m_Success );
    
        Assert::IsTrue(  sink->m_WaitCalls.load( ) == 1 );
        Assert::IsTrue(  sink->m_ResumeCalls.load( ) == 1 );
        Assert::IsTrue(  sink->m_LastWaitSeconds.load( ) >= 2 );
    }
