...
namespace ImageScraper
{
    ...
    {
    public:
        // The httpClient passed in MUST be the same RateLimitedHttpClient instance shared with
        // RedgifsUrlResolver - both hit the redgifs API and need a common bucket budget.
        RedgifsService( std::shared_ptr<JsonFile> appConfig, std::shared_ptr<JsonFile> userConfig, const std::string& caBundle, const std::string& outputDir, std::shared_ptr<IServiceSink> sink, std::shared_ptr<IHttpClient> httpClient, std::shared_ptr<IUrlResolver> urlResolver = nullptr );
