...
std::optional<std::string> ImageScraper::RedgifsUrlResolver::Resolve( const std::string& url )
{
    ...
    auto performLookup = [ & ]( ) -> std::optional<std::string>
    {
        ...
        if( result.m_Error.m_ErrorCode == ResponseErrorCode::Unauthorized )
        {
            ...
            if( !m_TokenCache.EnsureToken( ) )
            {
                LogError( "[%s] Could not reacquire redgifs token for slug '%s'", __FUNCTION__, slug.c_str( ) );
                return std::nullopt;
            }
            ...
            LogError( "[%s] redgifs lookup failed for slug '%s' after reauth: %s", __FUNCTION__, slug.c_str( ), retry.m_Error.m_ErrorString.c_str( ) );
            return std::nullopt;
        }
    ...
    const std::optional<std::string> response = performLookup( );
    if( !response.has_value( ) )
    {
        return std::nullopt;
    }

    const std::string resolved = RedgifsUtils::ExtractMediaUrlFromGifResponse( *response );
    if( resolved.empty( ) )
    {
